...
void func(int i, std::promise<int> p) {
    p.set_value(i + 123);  // пишем 123 в promise
}
...
int main() {
    ...
    std::thread thr(func, 1, std::move(p));
    // в thread передаем promise, чтобы thread знал, куда записать
